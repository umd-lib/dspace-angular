...
export class LogInCasComponent implements OnInit {
  ...
  /**
   * The CAS authentication location url.
   * @type {string}
   */
  ...
  redirectToCas() {
    ...
    this.authService.getRedirectUrl().pipe(take(1)).subscribe((redirectRoute) => {
      ...
      let casServerUrl = this.location;
      ...
      if (isNotNull(redirectUrlFromServer) && redirectUrlFromServer !== correctRedirectUrl) {
        ...
        casServerUrl = this.location.replace(/\?redirectUrl=(.*)/g, newRedirectUrl);
      }

      // redirect to CAS authentication url
      this.hardRedirectService.redirect(casServerUrl);
    });
