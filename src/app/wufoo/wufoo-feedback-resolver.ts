...
export class WufooFeedbackResolver implements Resolve<any> {
   ...
   */
  resolve(): void {
    const referringUrl$: Observable<string> = this.routeService.getCurrentUrl().pipe(
      take(1),
    ...
    const wufooFeedbackEndpoint$: Observable<string> = this.halService.getEndpoint('wufoo-feedback');

    const wufooFeedbackResponse$ = combineLatest([referringUrl$, wufooFeedbackEndpoint$]).pipe(
    ...
    wufooFeedbackResponse$.subscribe((obj) => {
      const wufooFeedbackResponse = <WufooFeedbackResponse> <unknown> obj;
      const redirectUrl = wufooFeedbackResponse.wufooFeedbackFormUrl;
